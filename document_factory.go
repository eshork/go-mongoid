...
func makeDocument(docType *collectionHandle, srcDoc bson.M) IDocument {
	...
	retAsIDocumentBase := ret.Interface().(IDocument)                        // convert into a IDocument interface
	retAsIDocumentBase.initDocumentBase(docType, retAsIDocumentBase, srcDoc) // call the self init
	return retAsIDocumentBase
}

// initDocumentBase configures this IDocument with a self-reference and an initial state
